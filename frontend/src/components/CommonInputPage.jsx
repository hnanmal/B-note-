...
export default function CommonInputPage({ title = 'Common Input Setting', apiBaseUrl }) {
  return (
    <div style={{ height: '100%', background: '#ffffff', borderRadius: 14, padding: 24, boxShadow: '0 10px 30px rgba(15,23,42,0.08)', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <header>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#475467' }}>{title}</div>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <CommonInputManager apiBaseUrl={apiBaseUrl} />
      </div>
    </div>
  );
}
